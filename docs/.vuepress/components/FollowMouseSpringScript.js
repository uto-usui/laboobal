...
class FollowMouseSpring {
  constructor($target, $wrapper = window, power = 0.8) {
    this.$target = $target
  ...
  update() {
     ...
     * @type {number}
     */
    this.springCoefficient.x +=
      (this.mouse.x - this.targetPosition.x) * this.power
    this.springCoefficient.y +=
      (this.mouse.y - this.targetPosition.y) * this.power
